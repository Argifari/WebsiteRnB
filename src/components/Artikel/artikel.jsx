...
const Artikel = () => {
  ...
  const [items, setItems] = useState([]); // hanya item bertype "artikel"
  ...
  // helper: cek type artikel (case-insensitive + trim)
  const isArtikel = (item) => {
  ...
  // Fetch data dari API (dan simpan hanya yang type = artikel)
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Ambil hanya item type = "artikel"
        const onlyArtikel = data.filter(isArtikel);

        // optional: sort terbaru dulu berdasarkan publishedAt
        const sorted = [...onlyArtikel].sort((a, b) => {
          const ta = new Date(a.publishedAt || 0).getTime();
          const tb = new Date(b.publishedAt || 0).getTime();
          return tb - ta;
        });

        // Kalau sebelumnya modal kebuka tapi itemnya ternyata bukan artikel / hilang, tutup modal
  ...
  // Filter berdasarkan title/slug/description/content (type sudah pasti artikel)
  ...
  const openModal = (item) => {
    // Extra guard: modal hanya untuk artikel
    if (!isArtikel(item)) return;
    ...
    <div className="artikel-page">
      ...
      >
        <div className="artikel-hero-overlay">
          <p className="artikel-hero-kicker">Wawasan & Informasi</p>
          <h1 className="artikel-hero-title">ARTIKEL</h1>
          <p className="artikel-hero-subtitle">
            Jelajahi konten terbaru seputar teknologi, pendidikan, dan
            pengembangan diri.
          </p>
        </div>
      </div>
            ...
            <div className="status-box status-error">
              ...
              <div className="status-subtitle">
                ...
                <ul className="status-list">
                  <li>
                    Server mock berjalan (Mockoon / JSON Server / backend)
                  </li>
          ...
          <div className="artikel-grid">
            {!loading && !errorMsg && filteredItems.length > 0
              ? filteredItems.map((item) => (
                  <article
                    key={item.id ?? `artikel-${item.slug}`}
                    className="artikel-card artikel-card-clickable"
                    role="button"
                    tabIndex={0}
                    ...
                    aria-label={`Buka artikel: ${item.title}`}
                  >
                    <div className="artikel-card-image-wrapper">
                      <img
                        src={
                          item.image ||
                    ...
                    <div className="artikel-card-body">
                      <span className="artikel-tag">artikel</span>
                      <h3 className="artikel-card-title">
                        {item.title || "-"}
                      </h3>
          ...
          >
            <div className="modal-header">
              <div className="modal-badges">
                <span className="modal-badge">artikel</span>
            ...
            <div className="modal-hero">
              <img
                ...
                alt={selectedItem.title || "Artikel"}
